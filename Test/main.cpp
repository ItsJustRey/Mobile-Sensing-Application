...
int sc_main(int argc, char *argv[]){
	...
	sc_signal<sc_int<16> >	randX_sig[NUM_MOBILES];
	sc_signal<sc_int<16> >	randY_sig[NUM_MOBILES];
	sc_signal<bool>			free_sig[NUM_MOBILES];						// SERVER --> MOBILE[i]
	sc_signal<bool>			packet_request_sig[NUM_MOBILES];				// SERVER <-- MOBILE[i]
	sc_signal<bool>			packet_permission_sig[NUM_MOBILES];			// SERVER --> MOBILE[i]
	...
	sc_signal<bool>			image_transmitted_done_sig[NUM_MOBILES];	// SERVER --> MOBILE[i]
	...
	for (int i = 0; i < NUM_MOBILES; i++)
	{
		...
		server.image_transmitted_done_out[i](image_transmitted_done_sig[i]);	// SERVER --> MOBILE[i]
	}
	...
	mobile0.image_transmitted_done_in(image_transmitted_done_sig[0]);	// SERVER --> MOBILE[i]
	// CREATE MOBILE 1
	...
	mobile1.image_transmitted_done_in(image_transmitted_done_sig[1]);	// SERVER --> MOBILE[i]
	// CREATE MOBILE 2
